...
const SANDBOX_TEMPLATES: Record<string, string> = {
  claude: 'docker.io/Auronexis/sandbox:claude-code',
  codex: 'docker.io/Auronexis/sandbox:codex',
};
...
function showSandboxHelp(output: CliOutput): void {
  output.log(`
${terminalTextStyles.bright('AURONEXIS Sandbox')} — Run AURONEXIS inside Docker Sandboxes

Usage:
  AURONEXIS sandbox <workspace>            Create and start a sandbox
  AURONEXIS sandbox <subcommand> [name]    Manage sandboxes
...
Subcommands:
  ...
  ${terminalTextStyles.bright('logs')}         Show AURONEXIS server logs
...
Examples:
  $ AURONEXIS sandbox ~/my-project
  $ AURONEXIS sandbox ~/my-project --agent codex --port 8080
  $ AURONEXIS sandbox ~/my-project --env SERVER_PORT=8080 --env HOST=0.0.0.0
  $ AURONEXIS sandbox ls
  $ AURONEXIS sandbox stop my-project
  $ AURONEXIS sandbox start my-project
  $ AURONEXIS sandbox rm my-project
...
Advanced usage:
  ...
  or passing prompts to the agent, use sbx directly with the template:
    ...
    sbx run --template docker.io/Auronexis/sandbox:claude-code claude ~/my-project --branch my-feature
    sbx run --template docker.io/Auronexis/sandbox:claude-code claude ~/project ~/libs:ro --memory 8g
...
function requireSandboxName(options: SandboxOptions, output: CliOutput): string | null {
  ...
  output.error(`\n${terminalTextStyles.error('❌')} Sandbox name required: AURONEXIS sandbox ${options.subcommand} <name>\n`);
  return null;
}
    ...
    async execute(argumentsList) {
      ...
      switch (options.subcommand) {
        ...
        case 'logs': {
          ...
          try {
            dependencies.runSandboxCommand(
              ['exec', sandboxName, 'bash', '-c', 'cat /tmp/AURONEXIS-ui.log'],
              true,
            );
          } catch (error) {
            dependencies.output.error(
        ...
        case 'start': {
          ...
          dependencies.output.log(`${terminalTextStyles.info('▶')} Launching AURONEXIS web server...`);
          dependencies.runSandboxCommand([
            'exec',
            sandboxName,
            'bash',
            '-c',
            'nohup AURONEXIS start --port 3001 > /tmp/AURONEXIS-ui.log 2>&1 & disown',
          ]);
          if (!publishSandboxPort(options, dependencies)) {
            return 1;
          }
          dependencies.output.log(`\n${terminalTextStyles.ok('✔')} ${terminalTextStyles.bright('AURONEXIS is ready!')}`);
        ...
        case 'create': {
          if (!options.workspace) {
            dependencies.output.error(
              `\n${terminalTextStyles.error('❌')} Workspace path required: AURONEXIS sandbox <path>\n`,
            );
            dependencies.output.log(`   Example: ${terminalTextStyles.bright('AURONEXIS sandbox ~/my-project')}\n`);
            return 1;
          }
          ...
          dependencies.output.log(`\n${terminalTextStyles.bright('AURONEXIS Sandbox')}`);
          ...
          dependencies.output.log(`\n${terminalTextStyles.dim('  Or install globally:')} npm install -g @AURONEXIS-ai/AURONEXIS\n`);
          return 0;
        }
