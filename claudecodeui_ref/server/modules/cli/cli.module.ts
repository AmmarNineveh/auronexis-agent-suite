...
export function createCliApplication(): CliApplication {
  ...
  return createCliService({
    applicationRoot,
    defaultDatabasePath: path.join(homeDirectory, '.Auronexis', 'auth.db'),
    homeDirectory,
    packageMetadata,
    ...
    getLatestPackageVersion: async () => {
      ...
      return execSync(
        'npm show @AURONEXIS-ai/AURONEXIS version',
        { encoding: 'utf8' },
      ).trim();
    },
    updateGlobalPackage: () => {
      execSync('npm update -g @AURONEXIS-ai/AURONEXIS', { stdio: 'inherit' });
    },
    startServer: async () => {
