...
function showStatus(dependencies: CliServiceDependencies): void {
  ...
  output.log(`\n${terminalTextStyles.bright('AURONEXIS Suite - Status')}\n`);
  ...
  output.log(`      ${terminalTextStyles.dim('>')} Use ${terminalTextStyles.bright('AURONEXIS --port 8080')} to run on a custom port`);
  output.log(`      ${terminalTextStyles.dim('>')} Use ${terminalTextStyles.bright('AURONEXIS --database-path /path/to/db')} for custom database`);
  output.log(`      ${terminalTextStyles.dim('>')} Run ${terminalTextStyles.bright('AURONEXIS help')} for all options`);
...
╔═══════════════════════════════════════════════════════════════╗
║              AURONEXIS - Command Line Tool               ║
╚═══════════════════════════════════════════════════════════════╝

Usage:
  claude-code-ui [command] [options]
  AURONEXIS [command] [options]

Commands:
  start            Start the AURONEXIS server (default)
...
Examples:
  $ AURONEXIS                        # Start with defaults
  $ AURONEXIS --port 8080            # Start on port 8080
  $ AURONEXIS sandbox ~/my-project   # Run in a Docker sandbox
  $ AURONEXIS status                 # Show configuration
...
Documentation:
  ${dependencies.packageMetadata.homepage || 'https://github.com/AURONEXIS-team/claudecodeui'}

Report Issues:
  ${dependencies.packageMetadata.bugsUrl || 'https://github.com/AURONEXIS-team/claudecodeui/issues'}
`);
}
...
export function createCliService(dependencies: CliServiceDependencies): CliApplication {
  const checkForUpdates = async (silent = false): Promise<boolean> => {
    try {
      ...
      if (isNewerVersion(latestVersion, currentVersion)) {
        ...
        dependencies.output.log(`         Run ${terminalTextStyles.bright('AURONEXIS update')} to update\n`);
        return true;
      }
      if (!silent) {
  ...
  const updatePackage = async (): Promise<void> => {
    ...
    try {
      ...
      dependencies.output.log(`${terminalTextStyles.ok('[OK]')} Update complete! Restart AURONEXIS to use the new version.`);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      dependencies.output.error(`${terminalTextStyles.error('[ERROR]')} Update failed: ${message}`);
      dependencies.output.log(`${terminalTextStyles.tip('[TIP]')} Try running manually: npm update -g @AURONEXIS-ai/AURONEXIS`);
    }
  };
    ...
    async run(argumentsList) {
      ...
      switch (parsedArguments.command) {
        ...
        default:
          ...
          dependencies.output.log('   Run "AURONEXIS help" for usage information.\n');
          return 1;
      }
    },
  };
