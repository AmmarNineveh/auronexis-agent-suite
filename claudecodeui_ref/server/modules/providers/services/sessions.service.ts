...
const MAX_AURONEXIS_SESSION_NAME_WORDS = 4;

function buildAURONEXISSessionName(initialMessage: string): string {
  const words = initialMessage.trim().split(/\s+/).filter(Boolean);
  return words.slice(0, MAX_AURONEXIS_SESSION_NAME_WORDS).join(' ') || 'Untitled Session';
}
...
export const sessionsService = {
   ...
   * comes directly from the first visible AURONEXIS message and is limited to
  ...
  createAppSession(
    ...
    const sessionName = buildAURONEXISSessionName(initialMessage);
