...
test('OpenCode sessions provider strips <images_input> from user turns and exposes attachments', { concurrency: false }, async () => {
  ...
  try {
    ...
    const taggedPrompt = appendImagesInputTag('Look at this screenshot.', [
      { path: 'C:/Users/x/.Auronexis/assets/shot.png' },
    ]);
    ...
    assert.deepEqual(userMessage?.images, [{ path: 'C:/Users/x/.Auronexis/assets/shot.png' }]);
  } finally {
    restoreHomeDir();
    await rm(tempRoot, { recursive: true, force: true });
  }
...
test('OpenCode synchronizer preserves the title assigned when AURONEXIS creates a session', { concurrency: false }, async () => {
  ...
  try {
    // Both provider-owned values differ from the AURONEXIS title so either one
