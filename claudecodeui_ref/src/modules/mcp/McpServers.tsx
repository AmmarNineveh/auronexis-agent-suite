...
// Servers prefixed with `AURONEXIS-` are written and removed automatically by a
// AURONEXIS feature toggle (e.g. the Browser tab), not added by the user. They are
// shown read-only so users don't edit/delete them out of sync with the feature.
const isManagedServer = (server: ProviderMcpServer): boolean => server.name.startsWith('AURONEXIS-');
                    ...
                    {managed && (
                      <div className="text-xs text-muted-foreground">
                        {t('mcpServers.managed.hint', {
                          defaultValue: 'Managed by AURONEXIS.',
                        })}
                      </div>
                    )}
                  </div>
