...
export class ViewHost {
  ...
  async showTabPlaceholder(tabId, target, message) {
    ...
    view.__AURONEXISStartupHtml = html;
    view.__AURONEXISLoadedUrl = null;
  }

  async showLocalStartupTarget(tabId, target, logs) {
    const view = this.getOrCreateTabView(tabId);
    if (view.__AURONEXISLoadingUrl) return;
    this.attach(view);
    const html = buildPlaceholderHtml(target.name || this.appName, 'Starting Local AURONEXIS...', logs);
    if (view.__AURONEXISStartupHtml === html) return;
    await view.webContents.loadURL(`data:text/html;charset=utf-8,${encodeURIComponent(html)}`);
    view.__AURONEXISStartupHtml = html;
    view.__AURONEXISLoadedUrl = null;
  }
  ...
  async showContentTarget(tabId, target) {
    ...
    const view = this.getOrCreateTabView(tabId);
    this.attach(view);
    if (target.forceLoad || view.__AURONEXISLoadedUrl !== target.url) {
      view.__AURONEXISLoadingUrl = loadUrl;
      try {
        await loadUrlWithTimeout(view.webContents, loadUrl);
        view.__AURONEXISLoadedUrl = target.url;
        view.__AURONEXISStartupHtml = null;
        delete target.loadUrl;
        delete target.forceLoad;
      } finally {
        if (view.__AURONEXISLoadingUrl === loadUrl) {
          view.__AURONEXISLoadingUrl = null;
        }
      }
    }
