...
/**
 * Installs the versioned local server runtime used by AURONEXIS Desktop.
 *
 * Server bundles are cached under:
 *   ~/.AURONEXIS/server/<version>/dist-server/server/index.js
 */

const DEFAULT_INSTALL_ROOT = path.join(os.homedir(), '.AURONEXIS', 'server');
const DEFAULT_BUNDLE_BASE_URL = 'https://github.com/AURONEXIS-team/claudecodeui/releases/download';
...
export class ServerInstaller {
  constructor({
    ...
    arch = process.arch,
    installRoot = process.env.AURONEXIS_SERVER_DIR || DEFAULT_INSTALL_ROOT,
    bundleBaseUrl = process.env.AURONEXIS_SERVER_BUNDLE_URL || DEFAULT_BUNDLE_BASE_URL,
    bundleReleaseTag = process.env.AURONEXIS_SERVER_BUNDLE_RELEASE_TAG || '',
    onLog,
  } = {}) {
  ...
  getBundleName() {
    return `AURONEXIS-local-server-${this.version}-${this.platform}-${this.arch}.tar.gz`;
  }
