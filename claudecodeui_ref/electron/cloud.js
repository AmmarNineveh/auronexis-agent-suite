...
export class CloudController {
  ...
  getEnvironmentUrl(environment) {
    return environment.access_url || `https://${environment.subdomain}.AURONEXIS.ai`;
  }
  ...
  async cloudApi(pathname, options = {}) {
    if (!this.cloudAccount?.apiKey) {
      throw new Error('Connect your AURONEXIS account first.');
    }
    ...
    try {
      ...
      if (error?.name === 'AbortError') {
        throw new Error(`AURONEXIS API request timed out after ${Math.round(CLOUD_API_TIMEOUT_MS / 1000)} seconds.`);
      }
      throw error;
    } finally {
    ...
    if (!response.ok) {
      ...
      throw new Error(body.error || `AURONEXIS API request failed: ${response.status}`);
    }
  ...
  buildConnectUrl() {
    ...
    connectUrl.searchParams.set('app_surface', 'AURONEXIS_desktop');
