...
function isAllowedPermissionOrigin(sourceUrl, controlPlaneUrl) {
  try {
    ...
    return source.origin === controlPlane.origin || source.hostname.endsWith('.AURONEXIS.ai');
  } catch {
    return false;
  }
}
...
export class DesktopWindowManager {
  ...
  emitDesktopState() {
    ...
    if (this.mainWindow && !this.mainWindow.webContents.isDestroyed()) {
      this.mainWindow.webContents.send('AURONEXIS-desktop:state-updated', state);
    }
    if (this.settingsWindow && !this.settingsWindow.webContents.isDestroyed()) {
      this.settingsWindow.webContents.send('AURONEXIS-desktop:state-updated', state);
    }
  }

  emitLauncherCommand(command) {
    if (!this.mainWindow || this.mainWindow.webContents.isDestroyed()) return;
    this.mainWindow.webContents.send('AURONEXIS-desktop:launcher-command', command);
  }

  emitSettingsCommand(command) {
    if (!this.settingsWindow || this.settingsWindow.webContents.isDestroyed()) return;
    this.settingsWindow.webContents.send('AURONEXIS-desktop:launcher-command', command);
  }
      ...
      return [
        {
          ...
          click: () => void this.actions.connectCloudAccount()
            .catch((error) => this.actions.showError('Could not connect AURONEXIS account', error)),
        },
      ];
    }
  ...
  buildAppMenu() {
    ...
    const cloudAccountLabel = cloudState.account?.apiKey
      ? (cloudState.account?.email ? `Connected: ${cloudState.account.email}` : 'AURONEXIS Connected')
      : (cloudState.account?.email ? `Reconnect: ${cloudState.account.email}` : 'Connect AURONEXIS Account...');
    ...
    const template = [
      ...
      },
      {
        ...
        submenu: [
          ...
          { type: 'separator' },
          {
            label: 'Open Local AURONEXIS',
            accelerator: 'CmdOrCtrl+L',
            click: () => void this.actions.openLocalInDesktop().catch((error) => this.actions.showError('Could not open local AURONEXIS', error)),
          },
          {
        ...
        submenu: [
          {
            ...
            click: () => void this.actions.connectCloudAccount().catch((error) => this.actions.showError('Could not connect AURONEXIS account', error)),
          },
          {
            label: 'Refresh Cloud Environments',
            click: () => void this.actions.refreshCloudEnvironments().catch((error) => this.actions.showError('Could not load AURONEXIS environments', error)),
            enabled: Boolean(cloudState.account?.apiKey),
          },
          {
            label: 'Logout AURONEXIS Account',
    ...
    const template = [
      {
        ...
        submenu: [
          {
            label: localState.localServerRunning ? 'Open Local in AURONEXIS' : 'Start Local in AURONEXIS',
            click: () => void this.actions.openLocalInDesktop().catch((error) => this.actions.showError('Could not open local AURONEXIS', error)),
          },
          {
