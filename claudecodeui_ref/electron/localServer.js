...
const SERVER_MARKER_PATH = path.join(os.homedir(), '.AURONEXIS', 'local-server.json');
const LOCAL_SERVER_URL_ENV_KEYS = [
  'AURONEXIS_DESKTOP_LOCAL_SERVER_URL',
  'AURONEXIS_LOCAL_SERVER_URL',
  'ELECTRON_LOCAL_SERVER_URL',
];
const LOCAL_SERVER_PORT_ENV_KEYS = [
  'AURONEXIS_DESKTOP_LOCAL_SERVER_PORT',
  'AURONEXIS_SERVER_PORT',
  'SERVER_PORT',
  'PORT',
];
...
async function isAURONEXISServer(baseUrl) {
...
async function waitForAURONEXISServer(baseUrl, timeoutMs) {
  const startedAt = Date.now();

  while (Date.now() - startedAt < timeoutMs) {
    if (await isAURONEXISServer(baseUrl)) {
      return true;
    }
...
export class LocalServerController {
  ...
  async resolveServerEntry() {
    ...
    if (process.env.AURONEXIS_USE_INSTALLED_SERVER !== '1' && await pathExists(bundledEntry)) {
      return bundledEntry;
    }
  ...
  startBundledServer(port, serverEntry) {
    ...
    this.ownedServerProcess.once('exit', (code, signal) => {
      ...
      if (this.ownedServerProcess) {
        console.error(`AURONEXIS desktop server exited with code ${code ?? 'null'} and signal ${signal ?? 'null'}`);
      }
      this.ownedServerProcess = null;
    });
  }
  ...
  async resolveLocalServerUrl() {
    ...
    if (devUrl) {
      const ready = await waitForAURONEXISServer(defaultUrl, SERVER_START_TIMEOUT_MS);
      if (!ready) {
    ...
    if (!forceOwnServer) {
      ...
      for (const candidateUrl of candidateUrls) {
        if (await isAURONEXISServer(candidateUrl)) {
          const displayUrl = getDisplayUrl(candidateUrl);
          this.localServerPort = getPortFromUrl(candidateUrl);
          this.appendStartupLog(`Using existing Local AURONEXIS at ${displayUrl}`);
          return displayUrl;
        }
      }
    }
    ...
    const ready = await waitForAURONEXISServer(serverUrl, SERVER_START_TIMEOUT_MS);
    if (!ready) {
    ...
    this.appendStartupLog(`Local AURONEXIS ready at ${displayUrl}`);
