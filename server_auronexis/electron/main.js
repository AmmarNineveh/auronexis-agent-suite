...
const APP_NAME = 'AURONEXIS';
const APP_USER_MODEL_ID = 'ai.AURONEXIS.desktop';
const CALLBACK_PROTOCOL = 'AURONEXIS';
const CALLBACK_URL = `${CALLBACK_PROTOCOL}://auth/callback`;
const AURONEXIS_CONTROL_PLANE_URL = process.env.AURONEXIS_CONTROL_PLANE_URL || 'https://AURONEXIS.ai';
...
function getCloudState() {
  return {
    ...
    controlPlaneUrl: AURONEXIS_CONTROL_PLANE_URL,
  };
}
...
async function hasCloudWebSession() {
  ...
  return cookies.some((cookie) => {
    ...
    return cookieDomain.includes('AURONEXIS.ai')
...
function isCloudAuthRedirect(url) {
  ...
  try {
    ...
    const controlPlane = new URL(AURONEXIS_CONTROL_PLANE_URL);
...
async function copyDiagnostics() {
  ...
  await dialog.showMessageBox(desktopWindow?.getMainWindow() || undefined, {
    ...
    message: 'AURONEXIS desktop diagnostics were copied to the clipboard.',
  });
}
...
async function refreshCloudEnvironments({ showErrors = false } = {}) {
  ...
  try {
    ...
    if (authState === 'expired') {
      const expiredError = new Error('Your AURONEXIS session expired. Reconnect your account.');
      if (showErrors) {
        await showError('AURONEXIS login required', expiredError);
        return [];
      }
      throw expiredError;
    }
    if (showErrors) {
      await showError('Could not load AURONEXIS environments', error);
      return [];
    }
    throw error;
  } finally {
...
async function handleDeepLink(url) {
  ...
  if (!pendingCloudConnectStartedAt || Date.now() - pendingCloudConnectStartedAt > AUTH_CALLBACK_TTL_MS) {
    await showError('AURONEXIS account connection failed', new Error('No recent AURONEXIS account connection was started from this app.'));
    return;
  }

  const apiKey = parsed.searchParams.get('api_key');
  if (!apiKey) {
    await showError('AURONEXIS account connection failed', new Error('The callback did not include an API key.'));
    return;
  }
  ...
  dialog.showMessageBox(desktopWindow?.getMainWindow() || undefined, {
    type: 'info',
    title: 'AURONEXIS account connected',
    message: cloud.getAccount()?.email ? `Connected as ${cloud.getAccount().email}.` : 'AURONEXIS account connected.',
  }).catch(() => {});
}
...
async function copyLocalWebUrl() {
  ...
  if (!shareableUrl) {
    throw new Error('Local AURONEXIS URL is not available yet.');
  }
  ...
  await dialog.showMessageBox(desktopWindow?.getMainWindow() || undefined, {
    ...
    detail: isLanUrl
      ...
      : `${shareableUrl}\n\nThis URL works on this computer. Enable LAN access before starting Local AURONEXIS to copy a phone-accessible URL.`,
  });
...
async function updateDesktopSetting(key, value) {
  ...
  if (result.requiresRestartNotice) {
    await dialog.showMessageBox(desktopWindow?.getMainWindow() || undefined, {
      ...
      detail: 'Quit AURONEXIS and stop the local server, then open Local AURONEXIS again.',
    });
  }
...
async function showEnvironmentPicker() {
  ...
  const choices = ['Local AURONEXIS', ...environments.map((environment) => {
  ...
  const response = await dialog.showMessageBox(desktopWindow?.getMainWindow(), {
    ...
    title: 'Switch AURONEXIS Environment',
...
function getSshTarget(credentials) {
  ...
  return `${credentials.username}@ssh.AURONEXIS.ai`;
}

function getSshHost(credentials) {
  const target = getSshTarget(credentials);
  const atIndex = target.indexOf('@');
  return atIndex >= 0 ? target.slice(atIndex + 1) : 'ssh.AURONEXIS.ai';
}
...
async function openCloudDashboard() {
  await openExternalUrl(AURONEXIS_CONTROL_PLANE_URL);
  return getDesktopState();
}
...
async function openEnvironmentInDesktop(environment) {
  ...
  if (environment.status !== 'running') {
    const response = await dialog.showMessageBox(desktopWindow?.getMainWindow(), {
      ...
      detail: 'AURONEXIS can start it before opening the remote app.',
    });
...
function getRemoteEnvironmentMenuItems() {
  ...
  if (!cloudAccount?.apiKey) {
    return [{ label: 'Connect AURONEXIS Account...', click: () => void connectCloudAccount() }];
  }
...
function registerIpcHandlers() {
  ipcMain.handle('AURONEXIS-desktop:connect-cloud', async () => ({
    ...getDesktopState(),
    connectUrl: await connectCloudAccount(),
  }));

  ipcMain.handle('AURONEXIS-desktop:copy-diagnostics', async () => {
    await copyDiagnostics();
    return getDesktopState();
  });

  ipcMain.handle('AURONEXIS-desktop:copy-local-web-url', async () => copyLocalWebUrl());
  ipcMain.handle('AURONEXIS-desktop:get-state', () => getDesktopState());
  ipcMain.handle('AURONEXIS-desktop:open-cloud-dashboard', async () => openCloudDashboard());
  ipcMain.handle('AURONEXIS-desktop:run-active-environment-action', async (_event, action) => runActiveEnvironmentAction(action));
  ipcMain.handle('AURONEXIS-desktop:open-environment', async (_event, environmentId) => {
  ...
  ipcMain.handle('AURONEXIS-desktop:open-local', async () => openLocalInDesktop());
  ipcMain.handle('AURONEXIS-desktop:open-local-web-ui', async () => openLocalWebUi());
  ipcMain.handle('AURONEXIS-desktop:refresh-environments', async () => {
    await refreshCloudEnvironments({ showErrors: true });
    return getDesktopState();
  });
  ipcMain.handle('AURONEXIS-desktop:disconnect-cloud', async () => clearCloudAccount());
  ipcMain.handle('AURONEXIS-desktop:reload-active-tab', async () => desktopWindow.reloadActiveTab());
  ipcMain.handle('AURONEXIS-desktop:show-environment-picker', async () => showEnvironmentPicker());
  ipcMain.handle('AURONEXIS-desktop:show-launcher', async () => {
    await desktopWindow.showLauncher();
    return getDesktopState();
  });
  ipcMain.handle('AURONEXIS-desktop:update-desktop-notifications', async (_event, settings) => {
    await desktopNotifications?.saveSettings(settings);
    return getDesktopState();
  });
  ipcMain.handle('AURONEXIS-desktop:show-desktop-settings', async () => desktopWindow.showDesktopSettings());
  ipcMain.handle('AURONEXIS-desktop:show-local-settings', async () => desktopWindow.showLocalSettings());
  ipcMain.handle('AURONEXIS-desktop:close-settings-window', async () => {
    desktopWindow.closeSettingsWindow();
    return getDesktopState();
  });
  ipcMain.handle('AURONEXIS-desktop:show-active-environment-actions-menu', async () => desktopWindow.showActiveEnvironmentActionsMenu());
  ipcMain.handle('AURONEXIS-desktop:show-environment-actions-menu', async (_event, environmentId) => desktopWindow.showEnvironmentActionsMenu(environmentId));
  ipcMain.handle('AURONEXIS-desktop:switch-tab', async (_event, tabId) => desktopWindow.switchDesktopTab(tabId));
  ipcMain.handle('AURONEXIS-desktop:close-tab', async (_event, tabId) => desktopWindow.closeDesktopTab(tabId));
  ipcMain.handle('AURONEXIS-desktop:update-setting', async (_event, key, value) => updateDesktopSetting(key, value));
}
...
if (registerSingleInstance()) {
  bootstrap().catch(async (error) => {
    await showError('AURONEXIS failed to start', error);
    app.quit();
  });
}
