...
app.use(express.urlencoded({ limit: '50mb', extended: true }));
  ...
  app.get('/download', (req, res) => {
    const apkPath = path.resolve(APP_ROOT, '../app_flutter/build/app/outputs/flutter-apk/app-debug.apk');
    if (fs.existsSync(apkPath)) {
      res.download(apkPath, 'AgentMobilePro.apk');
    } else {
...
// Chat attachment upload/serving (global ~/.Auronexis/assets store, protected)
...
// images and general files in the global ~/.Auronexis/assets folder.
...
const LOCAL_SERVER_MARKER_PATH = path.join(os.homedir(), '.Auronexis', 'local-server.json');
...
async function startServer() {
    try {
        ...
        server.listen(SERVER_PORT, HOST, async () => {
            ...
            console.log(`  ${terminalTextStyles.bright('AURONEXIS Server - Ready')}`);
            console.log(terminalTextStyles.dim('═'.repeat(63)));
            console.log('');
            ...
            console.log(`${terminalTextStyles.tip('[TIP]')}  Run "AURONEXIS status" for full configuration details`);
            console.log('');
